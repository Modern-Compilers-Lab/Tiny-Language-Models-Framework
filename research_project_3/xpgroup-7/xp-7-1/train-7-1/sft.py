...
from 	SFTSampler 			import SFTContiguousSampler
...
train_sftcs = SFTContiguousSampler(DDIR+"val.txt", nb_train_examples, batch_size, block_size, automatic_stop=False)
...
val_sftcs = SFTContiguousSampler(DDIR+"test.txt", nb_eval_examples, eval_batch_size, block_size, automatic_stop=False)
...
# Create the model
model = GPT()
model.load_state_dict(torch.load("/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-5/xp-5-18/train-5-18/checkpoints/best-model.pth", map_location=device))
...
if master_process:
	...
	last_losses = {"train": -1 , "val": 1.1376930475234985}
