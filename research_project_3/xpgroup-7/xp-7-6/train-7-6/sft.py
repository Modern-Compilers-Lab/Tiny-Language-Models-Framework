...
# Initialize the sft sampler. Here we use the contiguous sampler
...
nb_train_examples = 25000
train_sftcs = SFTRandomSampler("/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-35/data-ds-35/filtered_arccp_traced_snippets.txt", nb_train_examples, batch_size, block_size, purge=False)
nb_batches = train_sftcs.compute_nb_batches()
nb_epochs = 2
max_iters = nb_epochs * nb_batches
...
eval_batch_size = 128
eval_interval = int(nb_batches / 20)
eval_iters = 100  # Number of iterations for evaluation
nb_eval_examples = None
val_sftcs = SFTRandomSampler(DDIR + "val.txt", nb_eval_examples, eval_batch_size, block_size, purge=False)
...
def estimate_loss():
	...
	for sftcs in [val_sftcs]:
		losses = torch.zeros(eval_iters)
		for k in range(eval_iters):
			past = time.time()
			X, Y, EOI_tokens_indices = sftcs.__next__()
			...
			with torch.autocast(device_type=device, dtype=torch.bfloat16):
				logits, loss = model(X, Y, EOI_tokens_indices)
			losses[k] = loss.item()
			present = time.time()
...
for iter in range(start_iter, max_iters):
	...
	xb, yb, EOI_tokens_indices = train_sftcs.__next__()

	xb = xb[ddp_rank * microbatch_size: (ddp_rank+1) * microbatch_size]
	yb = yb[ddp_rank * microbatch_size: (ddp_rank+1) * microbatch_size]
	EOI_tokens_indices = EOI_tokens_indices[ddp_rank * microbatch_size: (ddp_rank+1) * microbatch_size]
	...
	with torch.autocast(device_type=device, dtype=torch.bfloat16):
		logits, loss = train_model(xb, yb, EOI_tokens_indices)
