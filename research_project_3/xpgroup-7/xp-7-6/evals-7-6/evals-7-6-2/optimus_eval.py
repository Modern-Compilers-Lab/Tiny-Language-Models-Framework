...
model = GPT()
model.eval()
model.load_state_dict(torch.load("../../train-7-6/checkpoints/checkpoint_1.0477824709609294.pth", map_location = device))
model.to(device)
compiled_model = model #torch.compile(model, mode='default')
...
with open("/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-41/longer-snippets/longer-snippets-data/final_snippets.txt", "r") as f:
	test_data = f.read()
