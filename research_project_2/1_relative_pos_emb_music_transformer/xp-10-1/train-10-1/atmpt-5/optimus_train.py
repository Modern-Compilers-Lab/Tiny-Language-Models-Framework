...
n_embd 		= 448	# Embedding dimension
...
learning_rate = 7.5e-5 	# Initial Learning rate value
decay_lr = True
...
eval_iters = 150  # Number of iterations for evaluation
...
# Create the model
model = GPT()
model.load_state_dict(torch.load("../atmpt-4/checkpoints/checkpoint_1.85.pth", map_location=device))
...
optimizer.load_state_dict(torch.load("../atmpt-4/checkpoints/optimizer_1.85.pth", map_location=device))
...
for iter in range(start_iter, max_iters):
	...
	if master_process:
		...
		if tl_warmup_iters <= tl_max_warmup_iters:
			...
			log(f"|BATCH_LOSS: {micro_loss:.5f} | TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec. | ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | LR: {lr:.7f}  | SPD: {(present - past) * 1000 :.4f} ms/it.| ERT: {convert_seconds((max_iters-iter-1) * mean_deltatime)} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
			print(f"|BATCH_LOSS: {micro_loss:.5f}|TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec.|ITERS: {iter+1} / {max_iters}|EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f}|COMP: {(iter+1)/max_iters * 100:.2f}%|LR: {lr:.7f} |SPD: {(present - past) * 1000 :.4f} ms/it|ERT: {convert_seconds((max_iters-iter-1) * mean_deltatime)}|ET: {convert_seconds(present-train_start_time)}")
