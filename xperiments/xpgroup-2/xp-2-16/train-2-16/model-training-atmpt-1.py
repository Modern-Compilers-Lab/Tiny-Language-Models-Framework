# Model training
DDIR = "/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-6/datapreps-6/dataprep-6-1/data-dp-6-1/"
deviceid = 5
...
class Head(nn.Module):
	...
	def forward(self, x):
		...
		# Reshape for RoPE
		q = q.view(B, T, 1, -1)  # (B, T, 1, head_size)
		k = k.view(B, T, 1, -1)  # (B, T, 1, head_size)

		# Apply RoPE
		q = self.rope(q)  # (B, T, 1, head_size)
		k = self.rope(k)  # (B, T, 1, head_size)

		# Reshape back
		q = q.view(B, T, -1)  # (B, T, head_size)
		k = k.view(B, T, -1)  # (B, T, head_size)
		
		out = F.scaled_dot_product_attention(
			q.unsqueeze(1),  # (B, 1, T, head_size)
			k.unsqueeze(1),  # (B, 1, T, head_size)
			v.unsqueeze(1),  # (B, 1, T, head_size)
			attn_mask=None,
...
class GPT(nn.Module):
	...
	def __init__(self):
		...
		self.ln_f = nn.RMSNorm(n_embd, eps=1e-5) 
