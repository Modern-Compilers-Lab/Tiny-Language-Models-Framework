...
# I'mma hard code the test set path in this one since I want to compare with the model on CoT
DDIR  = "../../../../datasets/dataset-16/datapreps-16/dataprep-16-1/data-dp-16-1/"
deviceid = 1
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("../train-4-3/checkpoints/best-model.pth"))
model.to(device)


## Reading the test data
log("Reading the test data")
with open("../../../../datasets/dataset-10/datapreps-10/dataprep-10-1/data-dp-10-1/test.txt", "r") as f:
	test_data = f.read()
...
for i, example in enumerate(examples):
	...
	if (i+1) % checkpoint_interval == 0:
		...
		log(f"accuracy checkpoint ==> ITERS: {i+1} | hard-accuracy: {(hard_match_counter/(i+1))*100:.2f}% | soft-accuracy: {(soft_match_counter/(i+1))*100:.2f}% |")
