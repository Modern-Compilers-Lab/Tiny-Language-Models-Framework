...
## ATMPT-1 (on Kindi)
DIR = "/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-1-codeExecution-outputPrediction/xp-8-60MModel-120MSimplifiedSnippets-Dataset3-8AttentionHeads-8Blocks/train/"
DDIR = "/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-3/datapreps/dataprep-1/data/"
deviceid = 1

## Logging boilerplate
log_file = open(DIR+"model-training-atmpt-2.log", "w")
...
run = neptune.init_run(
	...
	tags = ["60M-model", "~200M-code-snippets", "Simplified"],
	description = "We train from scratch the 60M parameters model on hodhaifa ~ 200M dataset with simplified code snippets, 8 attention heads and 8 transformer blocks",
...
run["arch-hyperparams/n_embd"] = n_embd = 792	  # Embedding dimension
...
miles = [int(max_iters * m) - 1_725_000 for m in [0.7, 0.8, 0.9]]  # Milestones for learning rate decay as fractions of max_iters
...
model = GPT()
model.load_state_dict(torch.load("checkpoints/checkpoint_iter1725000_epoch0.93_2024-10-17_08-49.pth"))
m = model.to(device)
...
log(f'{date_hour} : iter {1_725_000:5d} <=> epoch {(block_size * batch_size * (1_725_000+1))/len(train_data):.5f} | train loss {losses["train"]:.4f} | val loss {losses["val"]:.4f}')
nb_degradations = 0


log("training ...")
for iter in range(1_725_000, max_iters):
	past = time.time()
	...
	if (iter+1) % eval_interval == 0:
		...
		now = datetime.datetime.now()
		date_hour = now.strftime("%Y-%m-%d_%H-%M")
		torch.save(model.state_dict(), f"{DIR}checkpoints/checkpoint.pth")
		with open(f"{DIR}checkpoint.info", "w") as f:
		...
		if losses["val"] < best_val_loss:
			best_val_loss = losses["val"]
			torch.save(model.state_dict(), f"{DIR}checkpoints/best-model.pth")
