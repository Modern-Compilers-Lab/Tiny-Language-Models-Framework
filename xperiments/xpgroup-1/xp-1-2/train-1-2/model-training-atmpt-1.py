# Model training
DDIR = "../../../../datasets/dataset-4-FullRandom-30MSnipppets-2Depth-1Subblocks-NoDivisionMultiplicationModulo/datapreps/dataprep-1-80Train10Val10Test-CharacterLevelTokenization/data/"
deviceid = 2
...
run = neptune.init_run(
	...
	tags = ["30M-model", "~30M-code-snippets", "2Depth", "1Subblocks", "NoDivisionMultiplicationModulo"],
	description = "We train from scratch the 30M parameters model on dataset-4 which has ~30M snippets with depth 2, subblocks 1, no division multipilication modulo operators",
...
for iter in range(max_iters):
	...
	if (iter+1) % eval_interval == 0:
		...
		now = datetime.datetime.now()
		date_hour = now.strftime("%Y-%m-%d_%H-%M")
		torch.save(model.state_dict(), f"checkpoints/checkpoint_iter{iter+1}_epoch{epoch:.2f}_{date_hour}.pth")
