...
config 		= OmegaConf.load("../xp-5-2.conf")
XP 			= config['neptune-id']
DDIR 		= "/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-17/datapreps-17/dataprep-17-1/data-dp-17-1/"
deviceid 	= 5
...
# Save the netpune runid
log("saving the neptune runid")
...
log("Setting the device to GPU if available, otherwise CPU")
...
run["arch-hyperparams/n_embd"] 		= n_embd 		= 180	# Embedding dimension
run["arch-hyperparams/n_head"] 		= n_head 		= 12	# Number of attention heads
run["arch-hyperparams/n_layer"] 	= n_layer 		= 12	# Number of transformer blocks
...
run["train-hyperparams/max_pseudo_epochs"] 		= max_pseudo_epochs = 1.7	# Number of pseudo-epochs to train for
...
for iter in range(max_iters):
	...
	log(f"|ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {present - past :.4f} s/it.| ERT: {convert_seconds((max_iters-iter-1) * (present-past))} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
	
	# If we reached eval_interval; evaluate the model on the train and val splits
	if (iter+1) % eval_interval == 0:
		...
		now = datetime.datetime.now()
		date_hour = now.strftime("%Y-%m-%d_%H-%M")
		log(f'checkpoint_{date_hour} : iter {iter+1:10d} <=> epoch {epoch} | train loss {losses["train"]:.10f} | val loss {losses["val"]:.10f}')
