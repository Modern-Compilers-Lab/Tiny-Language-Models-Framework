	...
	for _ in tqdm(range(max_new_tokens)):
		...
		with ctx:
			if idx_cond.shape[1] < block_size:
				...
				loc_tens = torch.ones((1, 512), dtype=torch.long, device=device).copy_(idx_cond)
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-5/xp-5-13/train-5-13/atmpt-2/checkpoints/checkpoint_1.9997824117664174.pth", map_location = device))
model.to(device)
...
with open("/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-5/xp-5-13/evals-5-13/evals-5-13-2-bis/traced_30PS_chatGPT_normalized.txt", "r") as f:
	test_data = f.read()
...
for i, example in enumerate(examples):
	...
	example_match = regex.match(example)
	
	example_prompt = example_match.group(1)
	...
	example_output = example_match.group(2)
	encoded_example_output = tpt.encode(example_output)
	log(" generating ...")
	generated_output = tpt.decode(generate(model, compiled_model, example_prompt_tensor, max_new_tokens = len(encoded_example_output) + 7)[0].tolist())[len(encoded_example_prompt):]
	...
	# 	elif (generated_output[j] == generated_output[j+1]) and generated_output[j] == '\n':
	# 		break
	# the example's code should stop where the j index stops
	...
	example_output = tpt.decode(encoded_example_output)
	example_output = ' '.join(example_output)

	# if hard match
	if generated_output == example_output:
		hard_match_counter += 1
		hard_match_successes["example_input"].append(example_prompt)
	...
	# elif soft checking
		...
		soft_match_successes["example_input"].append(example_prompt)
	...
	# else complete failure
		...
		failures_counter += 1
		failures["example_input"].append(example_prompt)
		failures["example_output"].append(example_output)
		failures["all-generated-output"].append(generated_output+"@")

	present = time.time()
	
	log(f"|ITERS: {i+1} / {len(examples)} | COMP: {(i+1)/len(examples) * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {present - past :.4f} s/it.| ERT: {convert_seconds((len(examples)-i-1) * (present-past))} | ET: {convert_seconds(time.time()-start_time)}", p_level = 1)
	log(f"|hard-accuracy: {hard_match_counter} = {(hard_match_counter/(i+1))*100:.2f}% | soft-accuracy: {soft_match_counter} = {(soft_match_counter/(i+1))*100:.2f}% |", p_level = 2)
