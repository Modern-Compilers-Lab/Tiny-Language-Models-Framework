...
def get_batch(split):
	...
	if iter <= epoch_iters:
		...
		idx = random.randint(0, len(data) - batch_nb_tokens - 1)
...
model.load_state_dict(torch.load("./checkpoints/checkpoint_2.15.pth", map_location=device))
model.to(device)
...
# Setting the best val loss from the last taining attempt
# best_val_loss = 0.3743828237 #last_losses["val"]
...
for iter in range(start_iter, max_iters):
	...
	if ((iter+1) % eval_interval == 0 or (iter == max_iters - 1)) and master_process:
		...
		torch.save(model.state_dict(), f"checkpoints/checkpoint_{2+epoch:.2f}.pth")
		torch.save(optimizer.state_dict(), f"checkpoints/optimizer_{2+epoch:.2f}:.pth")
		...
		with open(f"checkpoints/checkpoint_{2+epoch:.2f}.info", "w") as f:
