...
def estimate_loss():
	...
	for split in ['train', 'val']:
		...
		for k in range(eval_iters):
			...
			losses[k] = loss.item()
			present = time.time()
			if master_process: log(f"{split}>|ITERS: {k+1} / {eval_iters} | COMP: {(k+1)/eval_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {present - past :.4f} s/it.| ERT: {convert_seconds((eval_iters-k-1) * (present-past))} | ET: {convert_seconds(time.time()-eval_start_time)}", p_level = 2)
...
model.load_state_dict(torch.load("../attempt-2/checkpoints/best-model.pth", map_location=device))
model.to(device)
...
if master_process:
	...
	last_losses = estimate_loss() # {'train':10, 'val':10}
	# Settin the best val loss from the last taining attempt
...
# iter starts from 126080 as it is the iteration from which we want to resume our last training
# 126080 is the number of iterations corresponding to exactly one epoch (starting from wich the batch overfitting problem due to an error in get_batch started)
start_iter = 0
nb_time_samples = 0
for iter in range(start_iter, max_iters):
	...
	if master_process:
		...
		if warmup_iters <= max_warmup_iters:
