...
model = GPT()
model.eval()
model.load_state_dict(torch.load("../../train-5-11/attempt-2/checkpoints/best-model.pth"))
model.to(device)
...
for i, example in enumerate(examples):
	...
	log(" generating ...")
	generated_output = tpt.decode(model.generate(example_prompt_tensor, max_new_tokens = len(encoded_example_output) + 7)[0].tolist())[len(encoded_example_prompt):]
