...
## On Greene
DDIR  = "../../../../../datasets/dataset-25/datapreps-25/dataprep-25-1/data-dp-25-1/"
deviceid = 7
amp = True
...
n_embd = 320	  # Embedding dimension for 120M
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("../../train-5-11/attempt-2/checkpoints/best-model.pth", map_location = device))
model.to(device)
...
dumb = torch.ones((1,512), dtype=torch.long).to(device)
...
print('INFERING dumb 3')
for _ in tqdm(range(100)):
	dumb2 = torch.ones((1,550), dtype=torch.long).to(device)
	dumb3 = torch.ones((1, 512), dtype=torch.long, device=device).copy_(dumb2[:, -512:])
	compiled_model(dumb3)
...
for i, example in enumerate(examples):
	...
	log(" generating ...")
	generated_output = tpt.decode(generate(model, example_prompt_tensor, max_new_tokens = len(encoded_example_output) + 7)[0].tolist())[len(encoded_example_prompt):]
