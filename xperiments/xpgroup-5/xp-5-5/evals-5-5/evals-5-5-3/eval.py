...
## On Greene
DDIR  = "../../../../../datasets/dataset-21/datapreps-21/dataprep-21-1/data-dp-21-1/"
deviceid = 6
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("../../train-5-5/checkpoints/checkpoint.pth", map_location=device))
model.to(device)
...
for i, example in enumerate(examples):
	...
	log(" generating ...")
	with torch.autocast(device_type=f'cuda:{deviceid}', dtype=torch.bfloat16):
		generated_output = model.generate(example_prompt_tensor, max_new_tokens = len(encoded_example_output) + 7)[0].tolist()[len(encoded_example_prompt):]
	generated_output = tpt.decode(generated_output)
