...
class TinypyTokenizer():
	...
	def encode_to_file(self, input_file_path:str, output_file_path:str, max_tokens=None):
		...
		for example in examples:
			...
			try:
				...
				nb_tokens += len(tokenized_example)
				nb_examples += 1
				...
				if max_tokens:
					if nb_tokens >= max_tokens:
						...
						pbar.update(len(tokenized_example))
				else:
					pbar.update(1)
			
			except Exception:
				return(f"error token:{token}")
		print(f"nb_tokens: {nb_tokens}")
		print(f"nb_examples: {nb_examples}")
		output_file.close()
