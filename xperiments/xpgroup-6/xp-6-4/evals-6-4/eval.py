...
def generate(model, compiled_model, idx, max_new_tokens):
	...
	for _ in tqdm(range(max_new_tokens)):
		...
		idx_cond = torch.empty(idx_cond.shape, dtype=torch.long, device=device).copy_(idx_cond) # (B, T)
		# get the predictions
		with ctx:
			if len(idx_cond[0, :]) < block_size:
				logits, loss = model(idx_cond)
			else:
				logits, loss = compiled_model(idx_cond)
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-5/xp-5-11/train-5-11/attempt-2/checkpoints/best-model.pth", map_location = device))
model.to(device)
