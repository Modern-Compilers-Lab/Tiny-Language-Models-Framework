...
learning_rate = 6e-4 	# Initial Learning rate value
decay_lr = True
warmup_iters = int(0.1 * epoch_iters) 
...
eval_interval = int(max_iters * 0.1) # Evaluation interval
...
def get_batch(split):
	...
	if trace:
		opt_tensor_log.write(f'\n\nxb BEFORE SPLIT: {data[iter * batch_nb_tokens:iter * batch_nb_tokens+batch_nb_tokens]}\n\n')
		opt_tensor_log.write(f'\n\nyb BEFORE SPLIT: {data[iter * batch_nb_tokens+1:iter * batch_nb_tokens+batch_nb_tokens+1]}\n\n')
...
model.to(device)
if master_process:
	num_parameters_hr = human_readable(sum(p.numel() for p in model.parameters() if p.requires_grad))
...
if trace:
	opt_tensor_log = open(f'opt{ddp_rank}_tensor_log.log', 'w')

# Launch the training loop
for iter in range(max_iters):
	
	# __Do one forward backward pass__
	...
	if trace:
		opt_tensor_log.write(f'\n\nITER \n {iter}\n\n')
		opt_tensor_log.write(f'\n\nmodel.token_embedding_table.weight BEFORE anything: \n{model.token_embedding_table.weight}\n\n')
	...
	if trace: opt_tensor_log.write(f'\n\nmodel.token_embedding_table.weight.grad AFTER BACKWARD: \n{model.token_embedding_table.weight.grad}\n\n')
	
	if grad_clip != 0:
		torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip)
		if trace: opt_tensor_log.write(f'\n\nmodel.token_embedding_table.weight.grad AFTER CLIP: \n{model.token_embedding_table.weight.grad}\n\n')
	...
	if trace: opt_tensor_log.write(f'\n\nmodel.token_embedding_table.weight AFTER STEP: \n{model.token_embedding_table.weight}\n\n')
	...
	if master_process:
		if iter + 1 <= init_iters:
			log(f"INIT ITER TIME: {present-past}")
			log(f"|BATCH_LOSS: {loss_val:.5f} | TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec. | ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {(present - past) * 1000 :.4f} ms/it.| ERT: {convert_seconds((max_iters-iter-1) * (present-past))} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
			print(f"|BATCH_LOSS: {loss_val:.5f}|TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec.|ITERS: {iter+1} / {max_iters}|EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f}|COMP: {(iter+1)/max_iters * 100:.2f}%|RATE: {1/(present-past):.2f} it./s|SPD: {(present - past) * 1000 :.4f} ms/it|ERT: {convert_seconds((max_iters-iter-1) * (present-past))}|ET: {convert_seconds(present-train_start_time)}")
		else:
			deltatimes += present-past
			log(f"|BATCH_LOSS: {loss_val:.5f} | TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec. | ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {(present - past) * 1000 :.4f} ms/it.| ERT: {convert_seconds((max_iters-iter-1) * ((deltatimes)/(iter + 1 - init_iters)))} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
			print(f"|BATCH_LOSS: {loss_val:.5f}|TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec.|ITERS: {iter+1} / {max_iters}|EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f}|COMP: {(iter+1)/max_iters * 100:.2f}%|RATE: {1/(present-past):.2f} it./s|SPD: {(present - past) * 1000 :.4f} ms/it|ERT: {convert_seconds((max_iters-iter-1) * ((deltatimes)/(iter + 1 - init_iters)))}|ET: {convert_seconds(present-train_start_time)}")
		batch_log.write(f"BATCH_LOSS: {loss_val:.5f}\n")
		batch_log.flush()
	...
	if ((iter+1) % eval_interval == 0 or (iter == max_iters - 1)) and master_process:
		...
		log(f'checkpoint_{date_hour} : iter {iter+1:10d} | epoch {epoch:.2f} | train loss {losses["train"]:.10f} | val loss {losses["val"]:.10f}\n')
		
		# If degradation in loss, check for early stopping
