...
# If master process, prepare the loggging facilites
...
for iter in range(200):
	...
	opt_tensor_log.write(f'\n\nxb BEFORE SPLIT: \n {xb}\n\n')
	opt_tensor_log.write(f'\n\nyb BEFORE SPLIT: \n {yb}\n\n')
	xb = xb[ddp_rank * microbatch_size:(ddp_rank+1) * microbatch_size]
	yb = yb[ddp_rank * microbatch_size:(ddp_rank+1) * microbatch_size]
	opt_tensor_log.write(f'\n\nxb AFTER SPLIT: \n {xb}\n\n')
	opt_tensor_log.write(f'\n\nyb AFTER SPLIT: \n {yb}\n\n')
	# print the weights of the model before optimizer step
	...
	if master_process:
		if iter + 1 <= init_iters:
			...
			print(f"|BATCH_LOSS: {micro_loss:.5f}|TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec.|ITERS: {iter+1} / {max_iters}|EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f}|COMP: {(iter+1)/max_iters * 100:.2f}%|RATE: {1/(present-past):.2f} it./s|SPD: {(present - past) * 1000 :.4f} ms/it|ERT: {convert_seconds((max_iters-iter-1) * (present-past))}|ET: {convert_seconds(present-train_start_time)}")
			log(f"|BATCH_LOSS: {micro_loss:.5f} | TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec. | ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {(present - past) * 1000 :.4f} ms/it.| ERT: {convert_seconds((max_iters-iter-1) * (present-past))} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
		else:
			deltatimes += present-past
			print(f"|BATCH_LOSS: {micro_loss:.5f}|TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec.|ITERS: {iter+1} / {max_iters}|EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f}|COMP: {(iter+1)/max_iters * 100:.2f}%|RATE: {1/(present-past):.2f} it./s|SPD: {(present - past) * 1000 :.4f} ms/it|ERT: {convert_seconds((max_iters-iter-1) * ((deltatimes)/(iter + 1 - init_iters)))}|ET: {convert_seconds(present-train_start_time)}")
			log(f"|BATCH_LOSS: {micro_loss:.5f} | TPS: {int(batch_size*block_size/(present-past)):8} tokens/sec. | ITERS: {iter+1} / {max_iters} | EPOCHS: {(block_size * batch_size * (iter+1))/len(train_data):.2f} | COMP: {(iter+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {(present - past) * 1000 :.4f} ms/it.| ERT: {convert_seconds((max_iters-iter-1) * ((deltatimes)/(iter + 1 - init_iters)))} | ET: {convert_seconds(present-train_start_time)}", p_level = 1)
