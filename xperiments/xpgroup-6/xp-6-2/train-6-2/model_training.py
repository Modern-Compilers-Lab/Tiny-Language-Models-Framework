...
# log(f'{date_hour} : iter {0:5d} <=> epoch 0 | train loss {losses["train"]} | val loss {losses["val"]}')
nb_degradations = 0

tensor_log = open("tensor_log.log", "w")
log("training ...")
for iter in range(200):
	...
	loss.backward()
	# print to the log the gradients of the token embedding table of the model
	tensor_log.write(f'\n\nmodel.token_embedding_table.weight.grad AFTER BACKWARD: \n{model.token_embedding_table.weight.grad}\n\n')
	optimizer.step()
	# print the weights of the embedding table after optimizer step
