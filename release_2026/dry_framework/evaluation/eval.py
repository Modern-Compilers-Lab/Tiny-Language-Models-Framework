...
#@ Fixes vs legacy:
#@   1) Padding-mask alignment: slice from the RIGHT (-total_len:) so the mask
#@      matches K/V after sliding-window trim.
#@   2) Generation budget no longer clipped to block_size; replaced with a
#@      generous MAX_GEN_TOKENS ceiling so long traces aren't truncated mid-line.
#@   3) Early-stop in the decode loop once every sequence in the batch has emitted
#@      the '\n\n' end-of-snippet token.
...
# you don't really have to excecute anything, you should use scheduler.py instead
# but, if you want to debug the code and check if it works excute this command :
# python eval.py --cptpth "CKPT" --datpth "DAT" --tstpth "TST" --outpth "OUT"
# CKPT is the folder path to the chkpoints you wanna evaluate
# DAT  is the folder path to the training/test data (so that the script can find the vocab file)
# TST  is the file path to the testing .txt file (so the path should end with .txt)
# OUT  is the folder path to wherever you wanna save the result scores
...
# Hard ceiling for generation length (per batch). Pick something larger than
# the longest expected reference trace; sliding-window attention handles
# context older than block_size for free.
MAX_GEN_TOKENS = 4096
...
# ── Mirror ALL terminal output to a file ───────────────────────────────
import sys
...
_terminal_log = open(os.path.join(args.outpth, "terminal.log"), "w")
sys.stdout = Tee(sys.__stdout__, _terminal_log)
sys.stderr = Tee(sys.__stderr__, _terminal_log)
# ───────────────────────────────────────────────────────────────────────
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, past_kv=None, attention_mask=None):
        ...
        # Sliding-window: keep only the last block_size keys/values
        ...
        total_len = k.size(2)
        if past_kv is not None:
            # Inference Mode: Generating NEXT token (q has T=1)
        ...
        # Padding mask: K/V was trimmed to the LAST `block_size` entries, so the
        # mask must be sliced from the right too. attention_mask is (B, L) with
        # 0 for pad, 1 for real.
...
# -----------------------------------------------------------------------------
# Optimized Generation Logic (With Masking + Early-Stop)
# -----------------------------------------------------------------------------

@torch.no_grad()
def generate(model, compiled_model, idx_list, max_new_tokens, eos_token_id=None):
    """
    idx_list: List of 1D tensors (varying lengths).
    max_new_tokens: How many tokens to generate at most.
    eos_token_id: If given, decoding stops once every sequence in the batch has
                  emitted this token at least once.
    """
    ...
    # Trim mask to the last block_size after prefill (matches K/V sliding window)
    if attention_mask.size(1) > block_size:
    ...
    # 3. Decoding Loop
    pbar = tqdm(range(max_new_tokens), desc="Decoding Batch", leave=False)
    for _ in pbar:
        probs = F.softmax(next_token_logits, dim=-1)
        _, next_token = torch.max(probs, dim=1, keepdim=True)

        # Once a sequence has finished, keep emitting EOS so output stays a
        # rectangular tensor and downstream slicing/comparison is unaffected.
        ...
        # Extend the attention mask for the new token (real, so 1.0)
...
# torch.compile can struggle with dynamic masks + lists; eager is safe.
...
while batch_idx < len(examples):
    ...
    # No more block_size clip — let traces run to completion. Cap at MAX_GEN_TOKENS
    # as a safety net; early-stop on \n\n usually exits well before this.
    budget = min(max_output_len + 16, MAX_GEN_TOKENS)

    log(f"Batch {batch_idx//batch_size_infer}: Generating (budget={budget})...", p_level=0)

    generated_outputs = generate(
        model, compiled_model, tensors_list,
