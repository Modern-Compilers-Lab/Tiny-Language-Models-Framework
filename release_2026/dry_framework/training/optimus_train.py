...
# torchrun --standalone --nproc-per-node=4 optimus_train.py

# !!!! the number of nproc-per-node must be equal to the gpu array length
# in order to launch training runs without having to keep your terminal busy (and also to be able to quit the session while training still running the background)
# use the 'screen' command, create a new screen using 'screen -S screen_name' then to enter a screen (use screen -r screen_name)
# once in a screen, you execute your scripts, then quietly leave by clicking 'ctrl + a' then 'd' 
# to destroy a screen while you are in the screen, write an 'exit' command
# to see the list of all active screens 'screen -ls'   

#_____________________________IMPORTANT_HYPERPARAMETERS___________________________________

#use ctrl+f for quick access:
# deviceids : pick which Gpus to use for training (in kindi there are 8 gpus, numbered 0 to 7, use gpustat command to check)
# base_seed : change when you want to repeat the same experiement with multiple seeds
# data_path : path to the folder where the dataset lives
# Hyperparams : a set of hyperparameters setting the architecture of the model
# eval_interval : when setting it, divide by the number of checkpoints you want to save
...
# percentage = "1"
data_path = "../data_generation/data"
...
if master_process:
    os.makedirs(f"{script_dir}/checkpoints{base_seed}/", exist_ok=True)
    log_file = open(f"{script_dir}/checkpoints{base_seed}/train.log", "w")
...
for iter in range(0, max_iters):
    ...
    if ((iter+1) % eval_interval == 0 or (iter == max_iters - 1)) and master_process:
        ...
        save_path = f"{script_dir}/checkpoints{base_seed}"
