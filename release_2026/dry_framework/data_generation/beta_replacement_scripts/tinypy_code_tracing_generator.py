...
# python tinypy_code_tracing_generator.py  --num-processes 4 --num-snippets 1000000

#_____________________________IMPORTANT_HYPERPARAMETERS___________________________________

#use ctrl+f for quick access:
# rarity : sets the variable diversity percentage
# batch_size : sets the batch size to generate for each worker (the bigger, the bigger and slower each step is on the progress bar, but beneficial when 'rarity' is high)
# MAIN GLOBAL VARIABLES : a region in the code where most hyperparameters governing the data distribusion lives.
# GENERATOR HYPERPARAMETERS : a region where you can control string generation rules
# HIGH-LEVEL-FILTERING : region where you can filter out snippets based on high level rules like nb of statements ..etc


#_____________________________________CODE_______________________________________________


# sets the variable diversity percentage
rarity = 20/100
...
class TinyPyGenerator2:
    def __init__(self,arr,cte,max_string_length,nb_workers,worker_id = 0):
        # resetting all stats variables, and resetting the generator for each worker
    ...
    # converts integers to strings of length 'length'
    ...
    # resets the generator (for example when we reach the generation threshold)
    ...
    # LEGACY : splits all integers in this universe to two teams, team 0 and 1
    ...
    # ALSO LEGACY : splits all strings in this universe to two teams, team 0 and 1
    ...
    # generates any number between [-a,a] with a=10^(power_limit-1) , teamNb is unused
    def generate_number(self, power_limit,teamNb):
        x = random.randint(0, 10**power_limit - 1)
        sign = random.choice((-1, 1))
        self.NbSizeDist[len(str(x))-1] +=1
        return sign * x

    # genrerates any string, first picks the length randomly, then uses an LCG generator to avoid variable name repetition
    # the 'reserved' parameter contains 'illegal' variable names, like special python keywords : 'for, if, else ..etc'
    ...
    def initiate_code(self):
        ...
        while self.context_stack.top().keywords_queue:
            ...
            if self.keyword == "ASSIGNMENT":
                y = random.random()
                if y<0.3 and len(self.generated_variables)>0:
                    lhs = random.choice(self.generated_variables)
                else :
                    lhs = self.generate_string(self.variable_identifiers,self.reserved_words,0)
                
                while lhs in self.for_loop_variables:
                    if y<0.3 and len(self.generated_variables)>0:
                        lhs = random.choice(self.generated_variables)
                    else :
                        lhs = self.generate_string(self.variable_identifiers,self.reserved_words,0)
                ...
                if lhs not in self.generated_variables :
                ...
                self.if_count[self.context_stack.depth()]+=1
                # an index variable used for a for loop must not be already used (either as a generated variable for assignements and as an already used index variable in another for loop)
                #index_variable = random.choice(list(set(self.variable_identifiers)-set(self.generated_variables)-set(self.for_loop_variables)))
                index_variable = self.generate_string(self.variable_identifiers,self.reserved_words,0)
                while index_variable in self.generated_variables or index_variable in self.for_loop_variables:
                    index_variable = self.generate_string(self.variable_identifiers,self.reserved_words,0)
                self.for_loop_variables.append(index_variable)
                range_value = random.randint(1, 3)
                tabs = '\t' * (self.context_stack.depth())
                self.code_snippet = self.code_snippet + f'{tabs}for {index_variable} in range({range_value}):\n'
                self.context_stack.top().GID["block_line_count"] += 1
                self.line_count += 1
                self.context_stack.push(init_dict={"block_line_count": 0}, init_queue=deque())
    ...
    def construct_skeleton(self):
        ...
        if self.line_count < self.min_line_count:
            if self.context_stack.depth() == 0:
                keywords_sequence.append(random.choice(["ASSIGNMENT", "IF_STATEMENT","FOR_LOOP"]))

            elif self.context_stack.depth() < self.max_nesting_depth:
                if self.context_stack.top().GID.get("block_line_count") != 0:
                    keywords_sequence.append(random.choice(["ASSIGNMENT", "IF_STATEMENT", "UNINDENT","FOR_LOOP"]))
            ...
            if self.context_stack.depth() == 0:
                keywords_sequence.append(random.choice(["ASSIGNMENT", "IF_STATEMENT", "END","FOR_LOOP"]))
            
            elif self.context_stack.depth() < self.max_nesting_depth:
                if self.context_stack.top().GID.get("block_line_count") != 0:
                    keywords_sequence.append(random.choice(["ASSIGNMENT", "IF_STATEMENT", "UNINDENT", "END","FOR_LOOP"]))
                
                else:
                    new_keyword = random.choice(["ASSIGNMENT", "IF_STATEMENT", "END","FOR_LOOP"])
                    if new_keyword == "END":
    ...
    def generate_code_snippet(self):
        ...
        self.max_digit_count = 3 # numbers will have digits between 1 to max_digit_count
        self.max_nesting_depth = 2 # snippets will have indentations between 0 and max_nesting_dept
        self.line_count = 0 # this is a stat variable
        self.generated_variables = list() 
        self.for_loop_variables =  list()
        self.min_line_count = 5 # the smallest length of any snippet
        self.max_line_count = 10 # the biggest length of any snippet
...
class TinyPyCodeTracer:
    ...
    def trace_snippet(self, snippet):
        ...
        # Create the tracing environment, this code basically reads each snippet, and generates the entire trace
        tracing_env = """
...
class ParallelTinyPyCodeTracingGenerator:
    def __init__(self, num_snippets):
        ...
        self.max_string_length = 5 # variable names have length 1 to max_string_length
        self.arr = [[0, 0, 0, 0, 0] for _ in range(self.max_string_length)] # each variable name length has it's own independent genreator, there for it's own independent genreator hyperparameters
        self.cte = 123456789 # initial genreation seed

        # for each genreator (for each length), set the hyperparameters (a, c and m) so that it satisfies the Hull-Dobell theorem
        # which basically allows us to cycle through all possible combinations ONCE
        # you may notice that we are generating integers, we are doing that because it's just more convenient, we can easily switch to strings using the int_to_string() method
        # instead of using, a, c, and m variables for each 'length' we store everything in the arr matrix, which should keep track of all genreators.
    ...
    # method to give work to a worker
        ...
        while generated_count < batch_size and consecutive_duplicates < max_consecutive_duplicates:
            try:
                ...
                if error_type is not None:
                    ...
                    if worker_tpg2.line_count<=worker_tpg2.max_line_count and worker_tpg2.if_count[0] <=2 and worker_tpg2.if_count[1] <=2 and worker_tpg2.if_count[2] <=2 and worker_tpg2.if_count[3] <=2 and worker_tpg2.if_count[4] <=2 and worker_tpg2.if_count[5] <=2 and worker_tpg2.if_count[6] <=2 and worker_tpg2.if_count[7] <=2 and worker_tpg2.if_count[8] <=2 and worker_tpg2.if_count[9] <=2 and worker_tpg2.if_count[10] <=2:
    ...
    def create_corpus(self, num_processes=None):
        ...
        batch_size = 2000  # Snippets per batch per worker 
        ...
        # print the final generation stats
        print(f"\n=== FINAL RESULTS ===")
...
if __name__ == "__main__":
    ...
    # _______________ VERIFICATION ZONE, COMMENT OUT FOR FASTER EXECUTION ________________________

    import matplotlib.pyplot as plt
    root_analysis = "./data"
    # Read all generated snippets   
    with open(root_analysis+"/raw_ID.txt", "r") as f:
        all_code = f.read()
    ...
    for length in range(1,6):
        ...
        for snippet in snippets:
            ...
            strings_len4 = re.findall(r'\b[a-z]{%d}\b' % length, snippet)
        ...
        rarity_array[length-1] = 100*(len(counter_len4)/(26)**length)
        #print(len(counter_len4))

    file_path = root_analysis+'/stats.txt'

    try:
        with open(file_path, 'r') as f:
            data = json.load(f)

        # Extract the required numbers (defaulting to 0 if key is missing)
        value_errors = data.get('value_errors', 0)
        unbound_errors = data.get('unbound_errors', 0)
        other_errors = data.get('other_errors', 0)
        generated = data.get('generated', 0)
        string_size_dist = data.get('StringSizeDist', [])

        # # Calculate the sum
        # total_sum = value_errors + unbound_errors + other_errors + generated
        # actual_string_distribution = (total_sum/generated)*len(counter_len4)
        # print("generated :", generated)
        # print("unique apparent strings of length ",length," : ", len(counter_len4))
        # print("total snippets :", total_sum)
        # print("total estimated actual strings : ", actual_string_distribution)
        # print("saturation percentage (possibly achievable with this datasize): ", 100*actual_string_distribution/string_size_dist[length-1], " %")
        # print("saturation percentage to theory (rarity included): ", 100*actual_string_distribution/(rarity*(26**length)), " %")

        print("Variable diversity distribution: ", rarity_array)
