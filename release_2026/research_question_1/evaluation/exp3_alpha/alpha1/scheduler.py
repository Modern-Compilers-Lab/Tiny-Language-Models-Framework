...
#on every architecture
gpus = [2,3,4,5,6]
data_path = "/data/ia2921/Tiny_language_model_framework/1Datasets/experiment_reboot/alpha_1.X/data"
checkpoint_path = "/data/ia2921/Tiny_language_model_framework/2Training/experiment_reboot/exp3_alpha1/checkpoints1"
ood_path = "/data/ia2921/Tiny_language_model_framework/1Datasets/experiment_reboot/alpha_D"
evaluation_script_path = ["/data/ia2921/Tiny_language_model_framework/3Evaluations/experiment_reboot/exp3_alpha/alpha1/eval.py"]
# checkpoints = [
#  'checkpoint_0.10.pth',
#  'checkpoint_0.20.pth',
#  'checkpoint_0.30.pth',
...
partial = []
for task in tasks : 
    partial.append(ood_path+"/"+task+"/test.txt")
tmp = []
...
test_datasets_titles = ["ID","D6", "D7", "D8", "D9"]
...
while jobs1:
    for gpu in gpus:
        ...
        if gpu_free(gpu):
            ...
            subprocess.Popen(f"screen -dmS eval_job1_{ckpt}_{test_title}_{timestamp} bash -c 'source /home/ia2921/anaconda3/etc/profile.d/conda.sh && conda activate torch_env && CUDA_VISIBLE_DEVICES={gpu} python \"{evaluation_script_path[0]}\" --cptpth \"{ckpt_path}\" --datpth \"{data_path}/\" --tstpth \"{test_pth}\" --outpth \"{os.path.join(output_path, checkpoint_name)}\"; exit'", shell=True)
