...
from tinypy_code_tracer_BL_BPE_tokenizer import BPEByteTokenizer
# Note: Ensure this path is correct for your runtime environment
tpt = BPEByteTokenizer("/data/ia2921/Tiny_language_model_framework/3Evaluations/experiment_reboot/exp4_alpha/ALIBI_BPE/bpe_tokenizer.json")
...
while batch_idx < len(examples):
    ...
    for example in batch_examples:
        ...
        encoded_output = tpt.encode(output)
        if len(encoded_output) > max_output_len:
            max_output_len = len(encoded_output)
        outputs_list.append(output)  # Store the original string directly
    ...
    for prompt, generated_output, example_output, prompt_length in zip(prompts_list, generated_outputs, outputs_list, prompt_lengths):
        # Decode returns a string, so we find where the prompt ends in the decoded text
        decoded_full = tpt.decode(generated_output)
        decoded_prompt = tpt.decode(generated_output[:prompt_length])
        
        # Slice the generated output string
        generated_output_str = decoded_full[len(decoded_prompt):]
