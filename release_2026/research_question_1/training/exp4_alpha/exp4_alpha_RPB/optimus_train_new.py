...
def get_batch(split):
    ...
    # Pin memory and move to device asynchronously
...
#         self.rel_pos_bias = nn.Parameter(torch.zeros(block_size))
...
#         rel_idx = torch.clamp(rel_idx, min=0, max=block_size-1)
#         bias = self.rel_pos_bias[rel_idx]
...
#     def __init__(self, num_heads, head_size, block_size=block_size):
#         super().__init__()
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size, block_size=block_size):
        ...
        # Learned relative positional bias (one for each head)
        self.rel_pos_bias = nn.Parameter(torch.zeros(num_heads, block_size))
        self.register_buffer('causal_mask', torch.triu(torch.ones(block_size, block_size), diagonal=1).bool())
    ...
    def forward(self, x):
        ...
        pos_idx = torch.arange(T, device=x.device)
        rel_idx = pos_idx[:, None] - pos_idx[None, :]  # i - j
        rel_idx = torch.clamp(rel_idx, min=0, max=self.rel_pos_bias.size(-1)-1)
        
        # Extract bias and shape to (Heads, T, T)
        bias = self.rel_pos_bias[:, rel_idx] 
        ...
        # Multiply by Values and re-assemble heads
        out = attn_probs @ v
