...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, past_kv=None, attention_mask=None):
        ...
        # 1. Match Training Script Extraction Exactly
        # Training uses: .view(B, T, 3, n_head, head_size)
        qkv = self.c_attn(x)
        ...
        if past_kv is not None:
            # --- INFERENCE MODE (Decoding 1 token) ---
            # We are at position (total_len - 1). We attend to 0..total_len-1
            # relative_pos shape: (1, 1, 1, total_len)
            ...
            # Broadcast m (1, H, 1, 1) * dist (1, 1, 1, L) -> (1, H, 1, L)
        ...
        if attention_mask is not None:
            # mask is (B, total_len). Expand to (B, 1, 1, total_len)
        ...
        y = self.c_proj(y) # No dropout in eval usually, but kept for consistency
        return y, present_kv
