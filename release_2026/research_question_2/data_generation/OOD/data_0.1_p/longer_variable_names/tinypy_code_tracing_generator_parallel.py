...
class TinyPyGenerator2:
    ...
    def generate_string(self,alphabet,reserved,teamNb):
        ...
        self.VarSizeDist[index+5]+=1
        string = self.int_to_string(self.next_rand(index,self.arr,self.rarity,step=(self.nb_workers),cte=self.cte),index+1+5)
...
class ParallelTinyPyCodeTracingGenerator:
    def __init__(self, num_snippets):
        ...
        for i in range(0,self.max_string_length):
            m = 26**(i+6)
