...
- The output folders (datasets, checkpoints, inference and evaluation results) are **not** shipped in the repository. They are regenerated on demand at runtime by the scripts (via `os.makedirs(..., exist_ok=True)`), and are ignored through the root `.gitignore`. Everything is reproducible, so there is no need to commit the empty folder tree or an archive of it.
