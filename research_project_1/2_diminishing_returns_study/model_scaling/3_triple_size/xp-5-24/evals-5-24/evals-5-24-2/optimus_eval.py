...
n_embd = 880	  # Embedding dimension for 120M
...
model = GPT()
model.eval()
model.load_state_dict(torch.load("/data/yb2618/Tiny-Language-Models-Framework/xperiments/xpgroup-5/xp-5-24/train-5-24/atmpt-2/checkpoints/checkpoint_2.00.pth", map_location = device))
model.to(device)
...
with open("/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-43/data-ds-43/filtered_arccp_traced_snippets.txt", "r") as f:
	test_data = f.read()
...
examples = sorted(examples, key = lambda x: len(tpt.encode(regex.match(x).group(2))))
print("Done !")
