...
import glob
for model_path in glob.glob("../checkpoints/checkpoint*.pth"):
	print(model_path)
	epoch = float(model_path.split("/")[-1].split("_")[1].split(".p")[0])
	if epoch < 1.7:
		continue
	print("Evaluating checkpoint:", model_path, "   of epoch:", epoch)
	...
	log(f'\nepoch {epoch:.2f} | train loss {losses["train"]:.10f} | val loss {losses["val"]:.10f}\n')
