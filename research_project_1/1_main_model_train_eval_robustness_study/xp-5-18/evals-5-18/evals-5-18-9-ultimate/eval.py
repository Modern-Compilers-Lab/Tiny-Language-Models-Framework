...
while batch_idx < len(examples):
	...
	for example in batch_examples:
		...
		tensors_list.append(torch.tensor(encoded_prompt, dtype=torch.long))
		
		output = match.group(2)
		tokenized_output = tpt.tokenize(output)
		outputs_list.append(" ".join(tokenized_output))
	
	batch_tensor = torch.stack(tensors_list, 0).to(device)
	
	log(" generating ...")
	generated_output = generate(model, compiled_model, batch_tensor, max_new_tokens = len(tokenized_output)+5).tolist()

	for prompt, generated_output, example_output in zip(prompts_list, generated_output, outputs_list):
		generated_output = " ".join(tpt.decode(generated_output)[len(encoded_prompt):])
