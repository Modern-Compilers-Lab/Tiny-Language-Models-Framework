...
cfg_rules = {
    ...
    # "ADVANCED_FOR_LOOP": ["FOR_LOOP",
						#   "FOR_HEADER NEW_LINE TAB_INDENT DISPLAY"],
...
# ADDITIONAL CODE FOR GENERATING FOR GENERATING THE CLASSICAL CODING PATTERN

def distribution_controller(authorized_statements,
							min_init, # the minimum number of initializations to create at the begining of the code
							min_length,
							max_length,
							line_counter,
							max_depth,
	...
	potential_keywords = set(authorized_statements)
...
def generate_random_code(min_init = 2,
	...
	# ADDITIONAL CODE FOR ENFORCING CLASSICAL CODE PATTERN
	no_loop_yet = True
	inside_loop = False
	...
	while new_pattern_line != "END":
		...
		# ADDITIONAL CODE FOR ENFORCING CLASSICAL CODE PATTERN
		if len(context_stack) - 1 == 0: # indentation level = 0
			if no_loop_yet:
				authorized_statements = set(pattern_vocabulary)
			else:
				authorized_statements = pattern_vocabulary - loop_statements
		elif len(context_stack) - 1 == 1: # indentation level 1
			if inside_loop:
				authorized_statements = pattern_vocabulary - loop_statements
			else:
				authorized_statements = non_indentation_statements
		else: # indentation level == 2
			authorized_statements = non_indentation_statements
		
		# We get the distribution from the distribution controller
		new_distribution = distribution_controller(authorized_statements, min_init, min_length, max_length, line_counter, max_depth, max_sub_blocks, context_stack)
		
		# We uniformly randomly choose a random keyword from the distribution 
		# new_pattern_line = random.choices(population = list(new_distribution.keys()), weights = list(new_distribution.values()))[0]
		new_pattern_line = random.choice(list(new_distribution.keys()))
		
		# ADDITIONAL CODE FOR ENFORCING CLASSICAL CODE PATTERN
		if new_pattern_line in loop_statements:
			no_loop_yet = False
			inside_loop = True
		...
		# Update the number of sub blocks in the context
	...
	# We append to the code_lines a display/advanced_display statement. Here we directly affect to code_lines[-1] since it is empty (development of "END" keyword)
...
if __name__ == "__main__":
	...
	parser.add_argument("--nb_programs", default = 60_000_000, help = "Number of programs to be generated")
	parser.add_argument("--output_file", default = "./data-ds-11/data.txt", help = "Number of programs to be generated")
	...
	parser.add_argument("--max_var_value", default = 1000, help = "The maximum value above which the absolute value of created variables must not go")
