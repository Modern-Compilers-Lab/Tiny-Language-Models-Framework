...
## Logging boilerplate
log_file = open("data-preping-atmpt-1.log", "w")
...
with open("data/train.txt", "r") as f, open("data/train.bin", "wb") as bin_file:
	...
	while True:
		...
		i = i+1
		present = time.time()
		#>> MISTAKE: i should remove the +1 form i+1 in COMP, otherwise it displays more than 100%
		log(f"|ITERS: {i} / {max_iters} | COMP: {(i+1)/max_iters * 100:.2f}% | RATE: {1/(present-past):.2f} it./s | SPD: {present - past :.4f} s/it.| ERT: {convert_seconds((max_iters-i) * (present-past))}", p_level = 1)

log("Reading and encoding val.txt directly to binary")
with open("data/val.txt", "r") as f, open("data/val.bin", "wb") as bin_file:
	chunk_size = 1024 * 1024 * 1000  # 1 GB
