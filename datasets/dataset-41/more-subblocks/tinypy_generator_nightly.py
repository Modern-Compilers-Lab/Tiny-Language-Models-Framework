...
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 999+1)]
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS	= [1 for i in range(1, 999+1)]
...
def queue_gen_actions():
	...
	if True:
		...
		####
		#### CONTROLLING THE MAX_ACHIEVED_SUBBLOCKS
		if keyword in ('WHILE_LOOP', 'SIMPLE_IF_STATEMENT'):
			new_nb_subblocks = context_stack[-1]["nb_blocks"] + 1
			if new_nb_subblocks > max_achieved_subblocks:
				max_achieved_subblocks = new_nb_subblocks
		####
...
if __name__ == "__main__":
	...
	parser.add_argument("--output_file"				, default = "./more-subblocks-data/direct_output_snippets.txt", help = "Number of programs to be generated")
...
try:
	...
	while nb_generated_programs < nb_programs:
		...
		# if we don't get at least 3 subblocks at some level, reject the code
		if max_achieved_subblocks <= 2:
			if use_tqdm:
				nb_rejections += 1
