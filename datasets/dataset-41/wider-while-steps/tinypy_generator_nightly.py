...
WHILE_LOOP_ADD_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(999+1)]
WHILE_LOOP_ADD_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(999+1)]
WHILE_LOOP_ADD_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(21, 40+1)]
WHILE_LOOP_ADD_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 for i in range(21, 40+1)]
WHILE_LOOP_ADD_UO_NB_ITERS									= [i for i in range(1, 10+1)]

# WHILE LOOP PARAMETERS FOR SUB UPDATE OPERATOR
WHILE_LOOP_SUB_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(999+1)]
WHILE_LOOP_SUB_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(999+1)]
WHILE_LOOP_SUB_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(21, 40)]
WHILE_LOOP_SUB_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 for i in range(21, 40)]
...
WHILE_LOOP_DIV_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(21, 25+1)]
...
WHILE_LOOP_FDIV_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 999+1)]
WHILE_LOOP_FDIV_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(1, 999+1)]
...
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 2+1)]
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS	= [1 for i in range(1, 2+1)]
...
WHILE_LOOP_MUL_UO_NB_ITERS_WEIGHTS_CONTROL_COEFFICIENT		= 0.01
...
DIGIT 							= [i for i in range(999+1)]
ARITHMETIC_OPERATORS 			= ["+", "-", "*", "//", "%"]
ARITHMETIC_OPERATORS_WEIGHTS 	= [1,1,1,1,1]
...
if __name__ == "__main__":
	...
	parser.add_argument("--nb_programs"				, default = 500, help = "Number of programs to be generated")
	parser.add_argument("--output_file"				, default = "./wider-while-steps-data/direct_output_snippets.txt", help = "Number of programs to be generated")
...
try:
	...
	while nb_generated_programs < nb_programs:
		...
		if not generated_while:
			nb_rejections += 1
			if use_tqdm:
				pbar.set_description(f"ZeroDiv: {nb_zero_divisions:,} |Overflows: {nb_var_value_overflows:,} |NameErrors: {nb_name_errors:,} |Timeouts: {nb_timeouts:,}| Rejections: {nb_rejections:,}")
