...
ALWAYS_EXECUTE_WHILE_LOOP = True
...
WHILE_LOOP_ADD_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(10+1)]
WHILE_LOOP_ADD_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(10+1)]
WHILE_LOOP_ADD_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(1, 10+1)]
WHILE_LOOP_ADD_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 if i!= 1 else 4 for i in range(1, 10+1)]
WHILE_LOOP_ADD_UO_NB_ITERS									= [i for i in range(1, 10+1)]

# WHILE LOOP PARAMETERS FOR SUB UPDATE OPERATOR
WHILE_LOOP_SUB_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(10+1)]
WHILE_LOOP_SUB_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(10+1)]
WHILE_LOOP_SUB_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(1, 10+1)]
WHILE_LOOP_SUB_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 if i!= 1 else 4 for i in range(1, 10+1)]
WHILE_LOOP_SUB_UO_NB_ITERS									= [i for i in range(1, 10+1)]

# WHILE LOOP PARAMETERS FOR DIV UPDATE OPERATOR
WHILE_LOOP_DIV_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 10+1)]
WHILE_LOOP_DIV_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(1, 10+1)]
WHILE_LOOP_DIV_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(2, 10+1)]
WHILE_LOOP_DIV_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 if i!= 10 else 3 for i in range(2, 10+1)]
...
WHILE_LOOP_FDIV_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 10+1)]
WHILE_LOOP_FDIV_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS 	= [1 for i in range(1, 10+1)]
WHILE_LOOP_FDIV_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(2, 10+1)]
WHILE_LOOP_FDIV_UO_UPDATE_OPERAND_VALUES_WEIGHTS 			= [1 if i!= 10 else 4 for i in range(2, 10+1)]
WHILE_LOOP_FDIV_UO_NB_ITERS									= [i for i in range(1, 10+1)]
WHILE_LOOP_FDIV_UO_NB_ITERS_WEIGHTS_CONTROL_COEFFICIENT		= 0.1

# WHILE LOOP PARAMETERS FOR MUL UPDATE OPERATOR
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES 			= [i for i in range(1, 10+1)]
WHILE_LOOP_MUL_UO_CONTROL_VARIABLE_INITIAL_VALUES_WEIGHTS	= [1 for i in range(1, 10+1)]
WHILE_LOOP_MUL_UO_UPDATE_OPERAND_VALUES 					= [i for i in range(2, 10+1)]
WHILE_LOOP_MUL_UO_UPDATE_OPERAND_VALUES_WEIGHTS				= [1 if i!= 10 else 4 for i in range(2, 10+1)]
...
def queue_gen_actions():
	...
	if True:
		...
		pk_weight = {
			...
			'DIRECT_ASSIGNMENT': 1,
			'SIMPLE_IF_STATEMENT': 10,
			'SIMPLE_ELIF_STATEMENT': 10,
			'ELSE_STATEMENT': 10,
			'WHILE_UPDATE': 10,
			'WHILE_LOOP': 10,
			'DISPLAY': 1,
...
if __name__ == "__main__":
	...
	parser.add_argument("--nb_programs"				, default = 750_000, help = "Number of programs to be generated")
	parser.add_argument("--output_file"				, default = "./data-ds-32-stage-1/arccp_direct_output_snippets.txt", help = "Number of programs to be generated")
	...
	###################################
	# Filling the hash set with the arccp examples from dataset-28
	print("Filling the hash set with the arccp examples from dataset-28")
	print("Reading the old data ...")
	with open('/data/yb2618/Tiny-Language-Models-Framework/datasets/dataset-28/data-ds-28/arccp_direct_output_snippets.txt', 'r') as f:
		old_data = f.read()
	print("Splitting the old data ...")
	old_examples = old_data.split('\n\n')[:-1]
	print("Hasing the old data ...")
	if use_tqdm:
		from tqdm import tqdm
		old_examples = tqdm(old_examples)
	for old_example in old_examples:
		hashes.add(hashlib.sha256(old_example.encode('utf-8')).hexdigest())
	###################################
